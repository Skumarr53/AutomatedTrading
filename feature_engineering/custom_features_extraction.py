...
class FeatureExtraction:
    ...
    def _add_high_low_features(self, data: pd.DataFrame):
        ...
        features = pd.DataFrame(index=data.index) if self.mode == "BACKTEST" else  pd.DataFrame()
        periods = {
        ...
        for label, window in periods.items():
            ...
            ) if self.mode == "BACKTEST" else [data['high'][-window:].max()]
            features[f'low_{label}'] = data['low'].rolling(window=window).min(
            ) if self.mode == "BACKTEST" else [data['low'][-window:].min()]
        return features
    ...
    def _add_candlestick_features(self, data: pd.DataFrame):
        ...
        return features if self.mode == "BACKTEST" else features.iloc[-1:]
    ...
    def _add_time_based_features(self, data: pd.DataFrame):
        ...
        if self.mode == "LIVE": data = data.iloc[-1:]
        features = pd.DataFrame(index=data.index) if self.mode == "BACKTEST" else  pd.DataFrame()
        features['hour_of_day'] = data['date'].dt.hour
        features['day_of_week'] = data['date'].dt.weekday
        features['month_of_year'] = data['date'].dt.month
        features['quarter_of_year'] = data['date'].dt.quarter
    ...
    def _add_gap_analysis_features(self, data: pd.DataFrame):
        ...
        if self.mode == "BACKTEST":
            ...
            data = data.iloc[-2:] 
            features['candlestick_gap'] = [(
                data['open'] - data['close'].shift(1)).iloc[-1]]
        
        return features

    def generate_features(self, data_dict: dict) -> dict:
        ...
        for symbol, data in data_dict.items():
            ...
            transformed_data_dict[symbol] = all_features
